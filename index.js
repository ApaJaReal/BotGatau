...
const vcard = 'BEGIN:VCARD\n' // JAN DI UBAH
            ...
            + 'FN: OWNER GANS\n' // NAMA KONTAK OWNER
            + 'ORG:Creator YoGi;\n' // NAMA CREATOR
            + 'TEL;type=CELL;type=VOICE;waid=6285784952920:+62 857-8495-2920\n' // NOMER HP LU
            + 'END:VCARD'
prefix = '#'
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285784952920@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"6285784952920@s.whatsapp.net"]
			const adminbotnumber = ["6285784952920@s.whatsapp.net"]
			const frendsowner = ["6285784952920@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*BoT-YoGi GROUP*\n\nLink : https://chat.whatsapp.com/GtFaNMh1crU1Ig9y8BG8xJ', text, { quoted: mek })
					break
					case 'funmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
    	...
    	if (!isUser) return(mess.only.userB)
     ...
     reply(teks)
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT Bot-YoGi ✨* 」\n*│+wa.me/6285784952920*╰──────*「 *BoT-YoGi* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  BoT-YoGi_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/Gv5W97v/IMG-20210106-WA1995.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6285784952920*╰──────*「 *BoT-YoGi* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER BoT-YoGi*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*Pulsa :* _081242508372_\n*Saweria :*_https://saweria.co/YOGIRG2_\n*Dana :* _belum tersedia_', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  BoT-YoGi 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * BoT-YoGi Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
